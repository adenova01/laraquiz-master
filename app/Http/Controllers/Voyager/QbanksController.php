...
class QbanksController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        $dataType_options = Voyager::model('DataType')->where('slug', '=', 'options')->first();

        $dataTypeContent_options =$this->laodOptions($id);
        if (strlen($dataType->model_name) != 0) {
            $model = app($dataType->model_name);
        ...
        foreach ($dataType_options->addRows as $key => $row) {
            $dataType_options->addRows[$key]['col_width'] = $row->details->width ?? 100;
        }
        // If a column has a relationship associated with it, we do not want to show that field
        ...
        //$options=$this->laodOptions($id);
        
        if (view()->exists("voyager::$slug.edit-add")) {
            $view = "voyager::$slug.edit-add";
        }

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','dataType_options','dataTypeContent_options'));
    }
    //laodOptions
    private function laodOptions($idQuestion){
        $options=Option::where('idq',$idQuestion)->get();
        return $options;
    }
    ...
    //
    //****************************************
    private function UpdateOptions(Request $request){
        $arrayOptions=[];
        for ($i=0; $i < count($request->post()['idoptions']); $i++) { 
            $arrayOptions[]=[
                'id'=>$request->post()['idoptions'][$i],
                'correct_answer'=>$request->post()['correct_answer'][$i],
                'score'=>$request->post()['score'][$i],
                'answer'=>$request->post()['answer'][$i]
            ];
        }
        
        foreach ($arrayOptions as $row) {
            $option=Option::find($row['id']);
            $option->correct_answer=$row['correct_answer'];
            $option->score=$row['score'];
            $option->answer=$row['answer'];
            $option->save();
        }
    }
    ...
    public function create(Request $request)
    {
        ...
        //$this->authorize('add', app($dataType_options->model_name));

        $dataTypeContent_options = (strlen($dataType_options->model_name) != 0)
            ? new $dataType_options->model_name()
            : false;
        ...
        $view = 'voyager::bread.edit-add';
        //$data_rows_options=data_row::where('data_type_id',6)->get();
        if (view()->exists("voyager::$slug.edit-add")) {
            $view = "voyager::$slug.edit-add";
        }
    ...
    public function store(Request $request)
    {
        ...
        //dd($request->post());
        $dataOptions = $request->post();
        $arrayoptions=[];
        $id=Qbank::all()->last();
        //dd($dataOptions);
        for($i=0; $i<count($dataOptions['correct_answer']); $i++){
            $arrayoptions[]=[
                'idq' =>$id->id,
                'answer' =>$dataOptions['answer'][$i],
                'score' =>$dataOptions['score'][$i],
                'correct_answer'=>$dataOptions['correct_answer'][$i]
            ];
        }
        foreach($arrayoptions as $row){
